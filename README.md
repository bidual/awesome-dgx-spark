...
- [eugr/spark-vllm-docker](https://github.com/eugr/spark-vllm-docker) - vLLM Docker for one to eight DGX Sparks over Ray or native PyTorch distributed, with 36 run-recipe.sh YAML recipes from Qwen3.6-35B-A3B to GLM-5.2-NVFP4.
...
- [nacyot/vllm-ds4f-gb10](https://github.com/nacyot/vllm-ds4f-gb10) - vLLM 0.27.1 patch set restoring long DeepSeek-V4-Flash sessions from disk, 118,784 of 119,549 tokens in about 3 s against a 70 s re-prefill, cold start 302 to 137 s.
...
- [hasso5703/dgx-spark-qwen38](https://github.com/hasso5703/dgx-spark-qwen38) - One-command SGLang service on GB10 for five switchable Qwen3.8 targets, 27B NVFP4 at 50 tok/s greedy median via DFlash2 and Flash-Next 176B at 262K context.
...
- [pangoleen/qwen3.8-27b-dgx-spark-dflash2](https://github.com/pangoleen/qwen3.8-27b-dgx-spark-dflash2) - Qwen3.8-27B NVFP4 on one DGX Spark at a DFlash2 draft budget of 16, 64-78 tok/s single stream and 387 aggregate at 32, falling to 38-41 tok/s at 260K context.
...
- [0xBakeer/deepseek-v41-flash-spark](https://github.com/0xBakeer/deepseek-v41-flash-spark) - DeepSeek-V4.1-Flash on one DGX Spark keeping a measured hot set of FP4 experts resident and streaming misses from NVMe, 2.68 tok/s unpruned and 18.98 at a 40% expert keep.
...
- [Baekpica/ds4-dfm-rs](https://github.com/Baekpica/ds4-dfm-rs) - Rust-host continuation of the ds4 engine with DGX Spark as release target, C binaries kept as oracles over a 60-cell migration gate, nine families up to K2-Horizon-375B.
...
- [rdaum/eider](https://github.com/rdaum/eider) - Rust and CUDA inference server for sm_121 NVFP4 with no tensor runtime under it, serving Qwen3.8-Flash-Next with the 51B PLE table paged from NVMe at about 620 tok/s cold prefill.
...
- [drowzeys/keys-vLLm.0.27-Qwen3.8-27B-ADay777Ablit-NVFP4-A4Q-NVFP4-KV-4M-KV-token-pool-MTP3-Single-DGX-Spark](https://github.com/drowzeys/keys-vLLm.0.27-Qwen3.8-27B-ADay777Ablit-NVFP4-A4Q-NVFP4-KV-4M-KV-token-pool-MTP3-Single-DGX-Spark) - Abliterated Qwen3.8-27B NVFP4 on one DGX Spark, FA2 NVFP4-KV back-ported to sm_121a for a 4.34M-token pool and 4.14x concurrency at 1M context, plus A4Q fp4-QK at 8 to 10% prefill.
...
- [jethac/vllm-gemma4-nvfp4-kv-repro](https://github.com/jethac/vllm-gemma4-nvfp4-kv-repro) - Repro for Gemma 4 returning garbage under `--kv-cache-dtype nvfp4` on sm_121, traced to FULL cudagraph capture around the head_dim 512 VO-split wrapper, with wheels of the fixed branch.
...
- [Libertai/vllm-sparse-mla-blackwell](https://github.com/Libertai/vllm-sparse-mla-blackwell) - Hand-written NoPE sparse-MLA kernel and vLLM plugin for sm_121 and sm_120, tiles sized to the 101,376-byte shared-memory ceiling, plus the NVFP4 MoE zero-activation-scale fix from issue 54189.
...
- [vcruz305/vllm-exl3](https://github.com/vcruz305/vllm-exl3) - Out-of-tree vLLM plugin for EXL3 trellis packs on sm_121, with native MoE decode kernels at 24.6 against 16.9 tok/s single stream on a 2-bit GLM-5.3-Flash pack.
- [VincentKaufmann/fp4-cuda-kernel](https://github.com/VincentKaufmann/fp4-cuda-kernel) - FP4 tensor-core GEMM as a one-line Python call on sm_121 through CUTLASS, with a pre-quantized weight cache at 85-129 TFLOPS and 1.4-2.4x BF16 `F.linear`.
...
- [colonel-otto/3x-dgx-spark-deepseek-v4-v0.25-anemll-baseline](https://github.com/colonel-otto/3x-dgx-spark-deepseek-v4-v0.25-anemll-baseline) - DeepSeek-V4-Flash across three DGX Spark at tensor-parallel 3, decode 6.7 to 20.2% faster than two nodes at n=30 per cell, confirmed on a third-party harness.
...
- [k3net/docai-evals](https://github.com/k3net/docai-evals) - Evidence repository for 15 GB10 serving experiments under R1 to R3 reproducibility levels, where Marlin beats the publisher-recommended MoE backend by 3.3x and NVFP4 doubles a counterparty-role error rate.
...
- [styles01/sparkrun-recipes](https://github.com/styles01/sparkrun-recipes) - Source-pinned SparkRun recipes for six exclusive lanes on one GB10, from Ling 3.0 Flash INT4 at 256K to DeepSeek-V4-Flash 0731 at 20 tok/s single stream.
- [ThinkCode/glm53-flash-2x-gb10-bench](https://github.com/ThinkCode/glm53-flash-2x-gb10-bench) - GLM-5.3-Flash NVFP4 against EXL3 on two GB10, ahead at one stream and down three quarters at six concurrent, with context depth ruled out as the variable.
...
- [ciprianveg/gb10-glm-5.2](https://github.com/ciprianveg/gb10-glm-5.2) - GLM-5.2 Int4-Int8Mix on eight GB10 nodes at TP8 with a native sm_121 rebuild worth 0.4-2.0% prefill and no decode gain, adopted only to enable NVFP4 MLA KV cache.
...
- [drowzeys/keys-1M-CTX-Inkling-Small-NVFP4-Dspark-NVFP4-KV-Cache-SGlang-SM121-optimized-on-Two-DGX-Sparks](https://github.com/drowzeys/keys-1M-CTX-Inkling-Small-NVFP4-Dspark-NVFP4-KV-Cache-SGlang-SM121-optimized-on-Two-DGX-Sparks) - NVFP4 KV cache added to the SGLang triton backend for Inkling-Small on two DGX Spark, a 3.12x pool at 1,082,627 tokens and 1M context, 23.9 tok/s prose single stream.
...
- [jakejharris/jspark3](https://github.com/jakejharris/jspark3) - GLM-5.3-Flash EXL3 4 bpw at TP3 on three DGX Spark, W8A16 Marlin trunk overlay worth 7.27% on code, 66.3 tok/s single stream and 251 at four streams.
...
- [magicbear/DeepSeek-V4.1-Flash-SGLang-DGX-Spark](https://github.com/magicbear/DeepSeek-V4.1-Flash-SGLang-DGX-Spark) - DeepSeek-V4.1-Flash under SGLang on four against eight DGX Spark, 71.0 to 78.3 tok/s single stream and 1.63-1.76x prefill, with throughput per node better at four.
...
- [MiaAI-Lab/DeepSeek-v4.1-Flash-DGX-Sparks](https://github.com/MiaAI-Lab/DeepSeek-v4.1-Flash-DGX-Sparks) - DeepSeek-V4.1-Flash on three DGX Spark under SGLang, the smallest layout that fits once the 189 GiB Engram tables move to NVMe, 37.9 tok/s with heads padded 64 to 96.
...
- [neko-legends/spark-bench](https://github.com/neko-legends/spark-bench) - Four DGX Spark as one TP4 cluster over four model lanes, Qwen3.8-Flash-Next at 600.5 tok/s aggregate over 16 streams and DeepSeek-V4.1-Flash at 51.1 tok/s single stream.
...
- [tonyd2wild/DeepSeek-V4.1-Flash-vLLM-DGX-Spark](https://github.com/tonyd2wild/DeepSeek-V4.1-Flash-vLLM-DGX-Spark) - DeepSeek-V4.1-Flash across four DGX Spark at TP4, 73.8 tok/s on code and 131.9 aggregate at six streams, a 1,070,168-token KV pool, vision and tool calls verified.
...
- [tonyd2wild/GLM-5.3-Int4-Int8Mix-TP4-4x-DGX-Spark](https://github.com/tonyd2wild/GLM-5.3-Int4-Int8Mix-TP4-4x-DGX-Spark) - GLM-5.3 743B as Int4-Int8Mix on four DGX Spark, 53.32 tok/s under DFlash2 against MTP-4, with NVFP4 KV at 293,447 tokens and 270K context.
...
- [yunwei37/dgx-spark-4-ring-no-switch](https://github.com/yunwei37/dgx-spark-4-ring-no-switch) - Ten configurations exercised on a four-Spark direct ConnectX ring with verdicts, from GLM-5.2 TP4 at 33.42 tok/s to a Qwen3.8-Flash-Next BF16 run that fails above 32,768 tokens.
